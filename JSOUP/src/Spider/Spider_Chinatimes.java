...
public class Spider_Chinatimes {
	...
	public static void main(String[] args) throws KeyManagementException, NoSuchAlgorithmException, IOException {
		...
		String Jurl ="http://www.chinatimes.com/realtimenews/260402";
	    ...
	    while(!LinkQueue.nuVisitedUrlEmpty() 
			...
			try {
				...
				Elements elements = doc.select("#page > div.wrapper > div.page_container.stack.clear-fix.realtimenews_ad > article > header > h1");
				Elements ClsEs = doc.select("#page > div.wrapper > div.page_container.stack.clear-fix.realtimenews_ad");
				Elements timeEs = doc.select("#page > div.wrapper > div.page_container.stack.clear-fix.realtimenews_ad > article > div.article_info.clear-fix > div.reporter");
				Elements contentEs = doc.select("#page > div.wrapper > div.page_container.stack.clear-fix.realtimenews_ad > article > article");
				Elements replynum = doc.select("#page > div.wrapper > div.page_container.stack.clear-fix.realtimenews_ad > article > div:nth-child(6) > div.article_star.clear-fix > div.art_click.clear-fix > span.num");
				

				if (elements.size() > 0 && timeEs.size() > 0 && contentEs.size() >0){
					
					String title = elements.get(0).getElementsByTag("h1").text();
					String Cls = ClsEs.get(0).getElementsByTag("h6").get(3).text();
					String date = timeEs.get(0).getElementsByTag("time").text().trim();
					...
					String reply = replynum.get(0).text();
				...
				System.out.println("NullPointerException: "+Jurl);
			}
