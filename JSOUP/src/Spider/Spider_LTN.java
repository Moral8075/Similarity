...
public class Spider_LTN {
	...
	public static void main(String[] args) throws KeyManagementException, NoSuchAlgorithmException, IOException {
		...
		String Jurl ="http://news.ltn.com.tw/list/society";
	    ...
	    while(!LinkQueue.nuVisitedUrlEmpty() 
			...
			try {
				...
				for(Element element : bodyelements) {
					...
					if (url.startsWith("http://news.ltn.com.tw/news/society/breakingnews/") ||
							url.startsWith("https://news.ltn.com.tw/news/society/breakingnews/")){
						links.add(url);
					}
				}
				...
				////////
				Elements ClsEs = doc.select("#main > div.content > div.guide.boxTitle > a:nth-child(2)");
				Elements elements = doc.select("#main > div.content > h1");
				Elements timeEs = doc.select("#newstext > span");
				Elements contentEs = doc.select("#newstext ");
				...
				if (elements.size() > 0 && timeEs.size() > 0 && contentEs.size() >0){
					...
					contentEs.select("#newspic").remove();
